  ...
  <div class="pt-20 pb-12" :class="isDark ? 'dark' : 'light'">
    <div class="container mx-auto px-4 max-w-6xl">
      ...
      <section class="mb-10 flex justify-center">
        <div class="glass-effect rounded-3xl p-8 w-full max-w-5xl">
        ...
        <div class="glass-effect rounded-3xl p-8 w-full max-w-5xl">
          ...
          <form @submit.prevent="submitApplication" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <!-- 左侧列或行内并排 -->
              <div>
                <label class="block text-sm mb-2 font-medium transition-colors" :class="isDark ? 'text-gray-200' : 'text-gray-800'">
              ...
              <div>
                <label class="block text-sm mb-2 font-medium transition-colors" :class="isDark ? 'text-gray-200' : 'text-gray-800'">
                  站点链接 <span class="text-red-500">*</span>
                </label>
                <input
              ...
              <div class="md:col-span-2">
                ...
                <div class="flex gap-4 items-center">
                  <div class="w-12 h-12 flex-shrink-0 rounded-xl bg-black/10 dark:bg-white/10 flex items-center justify-center overflow-hidden border border-gray-200 dark:border-gray-700">
                    <img v-if="form.logo_url" :src="form.logo_url" class="w-full h-full object-cover" @error="form.logo_url = ''" />
                    <span v-else class="text-xl opacity-50">🖼️</span>
                  </div>
                  <input
                    v-model="form.logo_url"
                    type="url"
                    placeholder="https://example.com/logo.png"
                    class="friend-input flex-1"
                  />
                </div>
              </div>
              ...
              <div class="md:col-span-2">
                ...
                <textarea
                  ...
                  rows="2"
                  placeholder="有什么想对站长说的？（选填）"
                  class="friend-input resize-none"
                ></textarea>
              </div>
            </div>

            <div class="pt-4 border-t" :class="isDark ? 'border-tokyo-night-blue/20' : 'border-gray-200'">
              <button
                type="submit"
                :disabled="isSubmitting"
                class="w-full md:w-auto md:px-12 py-3 rounded-xl font-medium text-white transition-all duration-300 disabled:opacity-60 friend-submit-btn mx-auto block"
              >
                {{ isSubmitting ? '正在提交...' : '提交友链申请' }}
              </button>
            </div>
          </form>

          <p v-if="feedback.message" class="mt-4 text-center text-sm font-medium" :class="feedback.success ? 'text-emerald-500' : 'text-red-500'">
            {{ feedback.message }}
          </p>
        </div>
      </section>
